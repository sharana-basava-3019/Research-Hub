...
EventSchema.methods.registerUser = async function(userId) {
  ...
  const targetUserId = (userId?._id || userId?.id || userId || '').toString();
  const existingRegistration = this.registrations.find(r => {
    const regUserId = (r.user?._id || r.user?.id || r.user || '').toString();
    return regUserId && targetUserId && regUserId === targetUserId;
  });
