...
const EditProject = () => {
  ...
  const fetchProjectData = async () => {
    try {
      ...
      setFormData({
        ...
        expectedOutcomes: Array.isArray(project.expectedOutcomes) ? project.expectedOutcomes.join('\n') : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.response?.data?.error || 'Failed to update project';
  ...
  if (!user) {
    return (
      <div className="container section text-center">
        ...
        <p className="text-muted">You must be logged in to edit a project.</p>
  ...
  return (
    <div className="container section">
      ...
      <div className="mb-4">
        <nav aria-label="breadcrumb">
          <p className="text-muted mb-0">
            ...
            {' > '}
            <Link to={`/projects/${id}`} className="text-decoration-none text-muted">{formData.title || 'Project'}</Link>
            {' > '}
            <span className="text-dark fw-semibold">Edit</span>
          </p>
        </nav>
      </div>
      ...
      <div className="row mb-4">
        <div className="col">
          <h1 className="display-5 fw-bold">Edit Project</h1>
          <p className="text-muted">Update the details of your research project.</p>
        </div>
      </div>
