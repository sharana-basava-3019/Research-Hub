let naturalModule = null;
let wordTokenizerInstance = null;

function getNatural() {
  if (!naturalModule) {
    naturalModule = require('natural');
  }
  ...
  static async checkPlagiarism(newProject, existingProjects, threshold = 0.75) {
    try {
      ...
      for (const existingProject of existingProjects) {
        ...
        if (String(existingProject._id || '') === String(newProject._id || '')) {
          continue;
        }
