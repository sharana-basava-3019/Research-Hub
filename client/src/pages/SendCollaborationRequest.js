...
const SendCollaborationRequest = () => {
  ...
  const handleProjectChange = (projectId) => {
    ...
    if (project && (project.owner?._id === user.id || project.owner === user.id)) {
      setCollaborationMode('invite');
      setMessage('I would like to invite you to collaborate on my project. Your expertise would be valuable, and I believe we can create something impactful together.');
    } else {
      setCollaborationMode('request');
                  ...
                  <div className="row g-3 g-md-4">
                    ...
                    <div className="col-12">
                      <label htmlFor="project" className="form-label fw-semibold" style={{ color: '#495057', fontSize: '0.9rem' }}>
                        ...
                        {collaborationMode === 'invite' ? 'My Project (Inviting to)' : 'Project (Requesting to join)'} <span className="text-danger">*</span>
                      </label>
                      <select
                        id="project"
                      ...
                      >
                        ...
                        {projects.map((proj) => (
                          <option key={proj._id} value={proj._id}>
                            {proj.title} {(proj.owner?._id === user.id || proj.owner === user.id) ? '(My Project)' : ''}
                          </option>
                        ))}
                      </select>
                      {projects.length === 0 && (
                      ...
                      {selectedProject && (
                        <small className="d-block mt-2" style={{ 
                          ...
                          {collaborationMode === 'invite' 
                            ? 'You own this project - sending invitation to join your team' 
                            : 'Requesting permission to collaborate on this project'}
                        </small>
                      )}
                    </div>
