...
exports.getPlatformStats = asyncHandler(async (req, res, next) => {
  ...
  const [totalUsers, totalProjects, totalCollaborations, projectsWithAttachments] = await Promise.all([
    ...
    Project.aggregate([
      ...
      { $group: { _id: null, totalDocs: { $sum: '$attachmentCount' } } }
    ])
  ]);

  const totalDocuments = projectsWithAttachments[0]?.totalDocs || 0;
...
function calculateMatchScore(currentUser, targetUser) {
  ...
  const currentInterests = Array.isArray(currentUser.researchInterests) ? currentUser.researchInterests : [];
  const targetInterests = Array.isArray(targetUser.researchInterests) ? targetUser.researchInterests : [];
  ...
  if (currentInterests.length > 0 && targetInterests.length > 0) {
    const commonInterests = currentInterests.filter(
      interest => targetInterests.includes(interest)
    );
    
    const interestScore = commonInterests.length / Math.max(currentInterests.length, 1);
...
async function getFallbackRecommendations(userId) {
  ...
  if (!user) {
    return { recommendations: [], method: 'fallback-none', totalCandidates: 0 };
  }

  const userInterests = Array.isArray(user.researchInterests) ? user.researchInterests : [];
  
  // Find users with similar research interests or other active researchers if none
  const query = { _id: { $ne: userId } };
  if (userInterests.length > 0) {
    query.researchInterests = { $in: userInterests };
  }

  let candidates = await User.find(query)
    .select('firstName lastName institution department designation researchInterests publications profilePicture username')
    .limit(50);

  // If no candidates matched specific interests, fetch general researchers
  if (candidates.length === 0 && userInterests.length > 0) {
    candidates = await User.find({ _id: { $ne: userId } })
      .select('firstName lastName institution department designation researchInterests publications profilePicture username')
      .limit(50);
  }
  ...
  const recommendations = candidates.map(candidate => {
    ...
    const candidateInterests = Array.isArray(candidate.researchInterests) ? candidate.researchInterests : [];
    const matchingInterests = candidateInterests.filter(
